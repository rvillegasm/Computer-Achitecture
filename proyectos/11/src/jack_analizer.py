...
from jack_vm_writer import VMWriter
...
def main(argv):
    ...
    if os.path.isdir(argv[1]):
        ...
        for file in jack_files:
            analize(os.path.join(path, file), sym_table)

    else:
        analize(argv[1], sym_table)

def analize(file_path, sym_table):
    """ 
    ...
    try:
        ...
        print(se)
        sys.exit(-1)
    # create the listener and vm writer
    vm_writer = VMWriter(out_file_path)
    listener = Listener(parser, vm_writer, sym_table)
